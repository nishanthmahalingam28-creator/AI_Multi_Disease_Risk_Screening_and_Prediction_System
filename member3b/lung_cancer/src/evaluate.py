"""
Lung Cancer Prediction Module - Evaluation and Diagnostic Utilities.

Implements reusable metric calculations and diagnostic visualization routines.
Specifically computes medical-grade diagnostic indicators including Sensitivity (Recall),
Specificity, Balanced Accuracy, Precision, F1-Score, ROC-AUC, PR-AUC, and Confusion Matrix.
"""
...
def calculate_classification_metrics(
    ...
    """
    Computes comprehensive classification metrics for lung cancer diagnosis.

    Target definitions:
        Positive class (1): Cancer Positive (YES)
        Negative class (0): Cancer Negative (NO)

    Args:
        y_true: Ground truth binary target vector.
        y_pred: Predicted discrete labels (0 or 1).
        y_prob: Predicted positive class probabilities (class 1).
    ...
    if y_prob is not None:
        ...
        p_curve, r_curve, _ = precision_recall_curve(y_true_arr, y_prob_arr, pos_label=1)
...
def plot_confusion_matrix(
    ...
    # Annotations with descriptive labels
...
def plot_roc_curve(
    ...
    plt.plot(
        ...
        color="#1f77b4",
        lw=2.5,
        label=f"Selected Model (ROC-AUC = {roc_auc:.4f})",
    )
